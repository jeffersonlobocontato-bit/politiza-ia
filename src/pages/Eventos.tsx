...
// ─── Formulário compartilhado (criar / editar / duplicar) ──────────────────────

type FormMode = 'criar' | 'editar' | 'duplicar';

interface EventoFormProps {
  modo: FormMode;
  eventoBase?: Evento | null;
  onSalvo: (eventoId: string) => void;
  onCancelar: () => void;
}
...
function EventoForm({ modo, eventoBase, onSalvo, onCancelar }: EventoFormProps) {
  const createEvento = useCreateEvento();
  const updateEvento = useUpdateEvento();
  const uploadBanner = useUploadEventoBanner();

  const isEditar = modo === 'editar';
  const isDuplicar = modo === 'duplicar';

  const [form, setForm] = useState(() => ({
    titulo: eventoBase ? (isDuplicar ? `${eventoBase.titulo} (cópia)` : eventoBase.titulo) : '',
    descricao: eventoBase?.descricao ?? '',
    data_inicio: toLocalDatetimeInput(eventoBase?.data_inicio),
    data_fim: toLocalDatetimeInput(eventoBase?.data_fim),
    local_nome: eventoBase?.local_nome ?? '',
    endereco: eventoBase?.endereco ?? '',
    municipio: eventoBase?.municipio ?? '',
    is_online: eventoBase?.is_online ?? false,
    link_online: eventoBase?.link_online ?? '',
    capacidade_maxima: (eventoBase?.capacidade_maxima ?? '') as string | number,
  }));

  const [slug, setSlug] = useState<string>(() => {
    if (isEditar && eventoBase) return eventoBase.slug;
    if (isDuplicar && eventoBase) {
      // gera novo slug a partir do título copiado para evitar conflito
      return gerarSlugEvento(eventoBase.municipio ?? '', eventoBase.data_inicio, `${eventoBase.titulo} copia`);
    }
    return '';
  });
  const [slugManual, setSlugManual] = useState<boolean>(isEditar || isDuplicar);

  const [temaPaletaId, setTemaPaletaId] = useState<TemaPaletaId>(
    (eventoBase?.tema_paleta_id as TemaPaletaId) ?? 'mint'
  );
  const [corCustom, setCorCustom] = useState(eventoBase?.tema_cor_primaria || '#2FA85A');
  const [corCustomEscura, setCorCustomEscura] = useState(eventoBase?.tema_cor_primaria_escura || '#1F8444');
  const [bannerFile, setBannerFile] = useState<File | null>(null);
  const [bannerPreview, setBannerPreview] = useState<string | null>(
    eventoBase?.imagem_capa_url ?? null
  );
  const [salvando, setSalvando] = useState(false);

  const handleBannerChange = (file: File | null) => {
    setBannerFile(file);
    setBannerPreview(file ? URL.createObjectURL(file) : eventoBase?.imagem_capa_url ?? null);
  };

  // auto-gera slug enquanto o usuário não editar manualmente (apenas criação)
  useEffect(() => {
    if (slugManual) return;
    if (!form.titulo || !form.data_inicio) return;
    setSlug(gerarSlugEvento(form.municipio, new Date(form.data_inicio).toISOString(), form.titulo));
  }, [form.titulo, form.municipio, form.data_inicio, slugManual]);
  ...
  const linkPreview = `${PUBLIC_BASE_URL}/${slugLimpo}`;

  const regenerarSlug = () => {
    if (!form.titulo || !form.data_inicio) {
      toast.error('Defina título e data antes de gerar a URL');
      return;
    }
    setSlug(gerarSlugEvento(form.municipio, new Date(form.data_inicio).toISOString(), form.titulo));
    setSlugManual(false);
  ...
  const handleSalvar = async () => {
    ...
    if (!slugLimpo || slugLimpo.length < 3) {
      toast.error('Informe uma URL (slug) com pelo menos 3 caracteres');
      return;
    }
    ...
    try {
      ...
      if (isEditar && eventoBase) {
        if (slugLimpo !== eventoBase.slug) {
          toast.warning('A URL mudou — links antigos compartilhados deixam de funcionar.');
        }
        let imagemUrl: string | null | undefined = undefined;
        if (bannerFile) {
          imagemUrl = await uploadBanner.mutateAsync({ eventoId: eventoBase.id, file: bannerFile });
        }
        await updateEvento.mutateAsync({
          id: eventoBase.id,
          ...payload,
          ...(imagemUrl !== undefined ? { imagem_capa_url: imagemUrl } : {}),
        });
        toast.success('Evento atualizado!');
        onSalvo(eventoBase.id);
      } else {
        // criar OU duplicar = INSERT
        const novo = await createEvento.mutateAsync({
          ...payload,
          status: 'rascunho',
          // se duplicando sem novo upload, reaproveita a URL da imagem original
          ...(isDuplicar && !bannerFile && eventoBase?.imagem_capa_url
            ? { imagem_capa_url: eventoBase.imagem_capa_url }
            : {}),
        });
        if (bannerFile) {
          const url = await uploadBanner.mutateAsync({ eventoId: novo.id, file: bannerFile });
          await updateEvento.mutateAsync({ id: novo.id, imagem_capa_url: url });
        }
        toast.success(isDuplicar ? 'Evento duplicado! Ajuste os detalhes e publique.' : 'Evento criado! Publique quando estiver pronto.');
        onSalvo(novo.id);
      }
    } catch (err: any) {
      const msg = String(err?.message ?? err);
      if (msg.includes('duplicate') || msg.includes('unique') || msg.includes('eventos_slug')) {
        toast.error('Esse endereço (URL) já está em uso — escolha outro slug.');
      } else {
  ...
  const titulo = isEditar ? 'Editar evento' : isDuplicar ? 'Duplicar evento' : 'Novo evento';
    ...
    <Card className="p-4 border-primary/40 bg-primary/5">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        ...
        <div className="sm:col-span-2">
          <label className="text-xs text-muted-foreground flex items-center gap-1.5 mb-1.5">
            <Link2 className="w-3.5 h-3.5" /> URL do evento (endereço público) *
          </label>
          <div className="flex items-center gap-1">
            <span className="px-2 py-2 rounded-l-lg border border-r-0 border-border bg-muted/30 text-[11px] text-muted-foreground font-mono">
              {PUBLIC_BASE_URL}/
            </span>
            <input
              className="flex-1 bg-background border border-border rounded-none px-3 py-2 text-sm font-mono focus:outline-none focus:ring-1 focus:ring-primary"
              value={slug}
              onChange={e => { setSlug(slugify(e.target.value)); setSlugManual(true); }}
              placeholder="ex: curitiba19-06-2026"
            />
            <button type="button" onClick={regenerarSlug}
              title="Gerar automaticamente a partir do município, data e título"
              className="px-2.5 py-2 rounded-r-lg border border-l-0 border-border bg-muted/30 hover:bg-accent text-muted-foreground">
          ...
          {isEditar && eventoBase && slugLimpo !== eventoBase.slug && (
            <p className="text-[10px] text-amber-400 mt-1">
              ⚠️ Mudar a URL invalida links já compartilhados com o slug anterior.
            </p>
          )}
        </div>
      ...
      <div className="flex gap-2 mt-4">
        <button onClick={handleSalvar} disabled={salvando}
          ...
          {salvando ? 'Salvando…' : isEditar ? 'Salvar alterações' : isDuplicar ? 'Criar cópia' : 'Criar evento'}
        </button>
        <button onClick={onCancelar} className="px-4 py-1.5 rounded-lg border border-border text-sm text-muted-foreground hover:bg-accent">
          Cancelar
        </button>
      </div>
    </Card>
...
function EventosLista({ onSelect }: { onSelect: (id: string) => void }) {
  ...
  const [formState, setFormState] = useState<{ modo: FormMode; eventoBase: Evento | null } | null>(null);
  ...
  const confirmarRemover = async (e: Evento) => {
    if (!window.confirm(`Remover o evento "${e.titulo}"? As inscrições associadas serão preservadas.`)) return;
    try {
      await deleteEvento.mutateAsync(e.id);
  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-end">
        <button onClick={() => setFormState(formState ? null : { modo: 'criar', eventoBase: null })}
      ...
      {formState && (
        <EventoForm
          modo={formState.modo}
          eventoBase={formState.eventoBase}
          onSalvo={(id) => { setFormState(null); onSelect(id); }}
          ...
          <Card key={e.id} className="p-4 hover:border-primary/40 transition-colors cursor-pointer" onClick={() => onSelect(e.id)}>
            ...
            <div className="flex items-center justify-between pt-2 border-t border-border">
              ...
              <div className="flex items-center gap-0.5">
                <button onClick={ev => { ev.stopPropagation(); setFormState({ modo: 'editar', eventoBase: e }); }}
                  className="p-1.5 rounded hover:bg-accent text-muted-foreground hover:text-foreground" title="Editar evento">
                  <Edit2 className="w-3.5 h-3.5" />
                </button>
                <button onClick={ev => { ev.stopPropagation(); setFormState({ modo: 'duplicar', eventoBase: e }); }}
                  className="p-1.5 rounded hover:bg-accent text-muted-foreground hover:text-foreground" title="Duplicar evento">
                  <CopyIcon className="w-3.5 h-3.5" />
                </button>
                <button onClick={ev => { ev.stopPropagation(); copiarLink(e.slug); }}
                  className="p-1.5 rounded hover:bg-accent text-muted-foreground hover:text-foreground" title="Copiar link de compartilhamento">
                  <Link2 className="w-3.5 h-3.5" />
                </button>
                <button onClick={ev => { ev.stopPropagation(); confirmarRemover(e); }}
                  className="p-1.5 rounded hover:bg-destructive/20 text-muted-foreground hover:text-destructive" title="Remover evento">
