...
BEGIN
  ...
  -- Primeira ação por autor (usada para "ativação" = 1ª ação registrada)
  ...
  leaders_agg AS (
    SELECT lb.id, lb.name, lb.total_score, lb.avg_score, lb.action_count,
           ...
           0 AS active_count,
           0 AS avg_lead_days,
           -- Para liderança: eficiência = 100% ações
  ...
  ------------------------------------------------------------------
  -- MICROS  (coordenadores municipais = nível 5)
  -- Subordinados diretos: lideranças cujo coordinator_id = micro.id
  ------------------------------------------------------------------
  all_micros AS (
  ...
  -- Lideranças diretas ativas (com ao menos 1 ação) por micro
  ...
  ------------------------------------------------------------------
  -- MACROS (coordenadores regionais = nível 3)
  -- Subordinados diretos: micros (nível 5) cujo supervisor_id = macro.id
  ------------------------------------------------------------------
  all_macros AS (
    SELECT cm.id, cm.name
    FROM public.campaign_members cm
  ...
  -- Micros diretos ativos (status=ativo E user_id com pelo menos 1 ação) por macro
