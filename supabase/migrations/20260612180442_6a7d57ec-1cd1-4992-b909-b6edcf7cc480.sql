...
BEGIN
  -- Authorization: admin master only
  ...
  -- Match each action to a leader (created by the same user)
  action_to_leader AS (
  ...
  -- Resolve micro and macro coordinators via supervisor chain
  coord_resolved AS (
    SELECT atl.*,
  ...
  -- Final macro: either the resolved macro member, or NULL (fallback by macroregion_id below)
  final_rows AS (
    SELECT cr.*,
  ...
  -- Fallback: actions whose leader has no coordinator chain pontuam direto na macrorregião
  macros_region_agg AS (
    SELECT ('region:' || macro_id) AS id,
